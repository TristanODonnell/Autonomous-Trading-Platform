...
# ---------------------------------------------------------------------------
# NYSE holiday calendar — extend this set whenever you add new backfill years.
# Weekend filter is applied separately (weekday() >= 5) so only named holidays
# that fall on weekdays need to be listed here.
# Half-days (e.g. day-after-Thanksgiving, Christmas Eve) are NOT listed —
# Alpaca returns however many bars traded; the missing-bar logic will flag
# the shortened session as partial, which is correct.
# ---------------------------------------------------------------------------
_US_MARKET_HOLIDAYS: frozenset[date] = frozenset(
    {
        # 2022
        # date(2022, 1, 1) — New Year's falls on Saturday, not observed on a weekday
        date(2022, 1, 17),  # MLK Day
        date(2022, 2, 21),  # Presidents Day
        date(2022, 4, 15),  # Good Friday
        date(2022, 5, 30),  # Memorial Day
        date(2022, 6, 19),  # Juneteenth (first year as federal holiday)
        date(2022, 7, 4),  # Independence Day
        date(2022, 9, 5),  # Labor Day
        date(2022, 11, 24),  # Thanksgiving
        date(2022, 12, 26),  # Christmas (observed — 25th is Sunday)
        # 2023
        date(2023, 1, 2),  # New Year's (observed — 1st is Sunday)
...
class MarketBackfillService:
    ...
    async def backfill(
        ...
        with start_span(
            ...
            for i, ((symbol, bar_date), chunk_bars) in enumerate(grouped.items(), start=1):
                if i % 50 == 0 or i == 1:
                    logger.info(
                        "backfill_progress",
                        extra={
    ...
    def _expected_minute_timestamps_for_date(self, bar_date: date) -> set[datetime]:
        ...
        if bar_date.weekday() >= 5 or bar_date in _US_MARKET_HOLIDAYS:
            return set()
