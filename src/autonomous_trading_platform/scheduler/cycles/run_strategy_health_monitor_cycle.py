...
JOB_NAME = "strategy_health_monitor_cycle"
COMPONENT = "scheduler.run_strategy_health_monitor_cycle"
...
def run_strategy_health_monitor_cycle(
    ...
    manifest = create_governance_manifest(
        ...
        run_id=run_id,
        job_name=JOB_NAME,
        governance_action="health_monitor",
        input_settings={
    ...
    def job() -> dict:
        try:
            result = StrategyHealthMonitor(session=session).run(
                run_id=str(run_id),
                rebalance_run_id=rebalance_run_id,
            )
    ...
    try:
        ...
        with (
            ...
            cycle_span.set_attribute("ratp.governance_action", "health_monitor")
            try:
                result = runner.run(
                    job_name=JOB_NAME,
...
if __name__ == "__main__":
    run_strategy_health_monitor_cycle()
