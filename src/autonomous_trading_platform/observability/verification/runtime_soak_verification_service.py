...
class RuntimeSoakVerificationService:
    ...
    def verify(
        ...
        *,
        ...
        checks = [
            ...
            self._check_metric_export(window_start=window_start, window_end=window_end),
            self._check_trace_export(window_start=window_start, window_end=window_end),
            self._check_loki_ingestion(window_start=window_start, window_end=window_end),
            self._check_governance_runtime(window_start=window_start, window_end=window_end),
            self._check_replay_runtime(window_start=window_start, window_end=window_end),
            self._check_long_running_runtime_integrity(
                window_start=window_start,
                window_end=window_end,
            ),
        ...
        report = RuntimeSoakVerificationReport(
        ...
        if self._persist_report:
            RuntimeSoakReportRepository(self._session).append_report(report)
        return report
    ...
    def _check_observability_signals(
        ...
        *,
        ...
        return self._check_metric_export(window_start=window_start, window_end=window_end)

    def _check_metric_export(
        self,
        *,
        window_start: datetime,
        window_end: datetime,
        ...
        required_metrics = {
            "ratp_ingestion_lag_seconds": "ingestion_lag_seconds",
            "ratp_symbols_expected": "symbols_expected",
            "ratp_symbols_received": "symbols_received",
            "ratp_trading_cycle_runs_total": "trading_cycle_runs",
        }
        import autonomous_trading_platform.observability.metrics as metrics_module

        missing_instruments = [
            metric_name
            for metric_name, attribute_name in required_metrics.items()
            if not hasattr(metrics_module, attribute_name)
        ]
        collector_config = _read_repo_file("infra/observability/otel-collector-config.yaml")
        config_has_metrics_pipeline = all(
            token in collector_config for token in ("metrics:", "receivers: [otlp]", "prometheus")
        )
        runtime_runs = self._repository.list_runtime_job_runs(
            window_start=window_start,
            window_end=window_end,
        )
        metadata = {
            ...
            "collector_config_has_metrics_pipeline": config_has_metrics_pipeline,
            "runtime_run_count": len(runtime_runs),
        }
        if missing_instruments or not config_has_metrics_pipeline:
            return self._failed(
                RuntimeSoakCheckName.METRIC_EXPORT,
                "Metric export verification is missing required instruments or collector wiring.",
                metadata,
            )
        if not runtime_runs:
            return self._warning(
                RuntimeSoakCheckName.METRIC_EXPORT,
                "Metric export is configured, but no runtime runs existed in the soak window.",
                metadata,
            )
        return self._passed(
            RuntimeSoakCheckName.METRIC_EXPORT,
            "Metric instruments and collector export wiring were verified.",
            metadata,
        )

    def _check_trace_export(
        self,
        *,
        window_start: datetime,
        window_end: datetime,
        ...
        collector_config = _read_repo_file("infra/observability/otel-collector-config.yaml")
        config_has_trace_pipeline = all(
            token in collector_config
            for token in ("traces:", "otlphttp/tempo", "receivers: [ otlp ]")
        )
        runtime_runs = self._repository.list_runtime_job_runs(
            window_start=window_start,
            window_end=window_end,
        )
        correlated_runs = [row for row in runtime_runs if row.correlation_id]
        metadata = {
            "collector_config_has_trace_pipeline": config_has_trace_pipeline,
            "runtime_run_count": len(runtime_runs),
            "correlated_runtime_run_count": len(correlated_runs),
        }
        if not config_has_trace_pipeline:
            return self._failed(
                RuntimeSoakCheckName.TRACE_EXPORT,
                "Trace export verification is missing OTLP-to-Tempo collector wiring.",
                metadata,
            )
        if runtime_runs and not correlated_runs:
            return self._failed(
                RuntimeSoakCheckName.TRACE_EXPORT,
                "Runtime jobs were recorded without correlation IDs needed for trace export.",
                metadata,
            )
        if not runtime_runs:
            return self._warning(
                RuntimeSoakCheckName.TRACE_EXPORT,
                "Trace export is configured, but no runtime runs existed in the soak window.",
                metadata,
            )
        return self._passed(
            RuntimeSoakCheckName.TRACE_EXPORT,
            "Trace export wiring and runtime correlation evidence were verified.",
            metadata,
        )

    def _check_loki_ingestion(
        self,
        *,
        window_start: datetime,
        window_end: datetime,
        ...
        collector_config = _read_repo_file("infra/observability/otel-collector-config.yaml")
        config_has_loki_pipeline = all(
            token in collector_config for token in ("logs:", "otlp_http/loki", "lgtm:3100")
        )
        audit_events = self._repository.list_audit_events_by_component_or_type(
            window_start=window_start,
            window_end=window_end,
            component_prefixes=("runtime", "scheduler", "governance"),
            event_type_patterns=("%FAILED%", "%PROMOTION%", "%DEMOTION%", "%REBALANCE%"),
        )
        metadata = {
            "collector_config_has_loki_pipeline": config_has_loki_pipeline,
            "audit_event_count": len(audit_events),
            "audit_event_types": [row.event_type for row in audit_events],
        }
        if not config_has_loki_pipeline:
            return self._failed(
                RuntimeSoakCheckName.LOKI_INGESTION,
                "Loki ingestion verification is missing OTLP log collector wiring.",
                metadata,
            )
        if not audit_events:
            return self._warning(
                RuntimeSoakCheckName.LOKI_INGESTION,
                "Loki ingestion is configured, but no runtime audit events existed in the window.",
                metadata,
            )
        return self._passed(
            RuntimeSoakCheckName.LOKI_INGESTION,
            "Loki ingestion wiring and runtime log evidence were verified.",
            metadata,
        )

    def _check_governance_runtime(
        self,
        *,
        window_start: datetime,
        window_end: datetime,
        ...
        jobs = self._repository.list_successful_job_runs_by_names(
            job_names=job_names,
            window_start=window_start,
            window_end=window_end,
        )
        completed_names = {row.job_name for row in jobs}
        transitions = self._repository.list_governance_transitions(
            window_start=window_start,
            window_end=window_end,
        )
        metadata = {
            "expected_governance_jobs": list(job_names),
            "completed_governance_jobs": sorted(completed_names),
            "missing_governance_jobs": sorted(set(job_names) - completed_names),
            "transition_count": len(transitions),
            "transition_states": sorted({row.current_state for row in transitions}),
        }
        if len(completed_names) == len(job_names):
            return self._passed(
                RuntimeSoakCheckName.GOVERNANCE_RUNTIME,
                "Governance promotion, demotion, and allocation rebalance jobs completed.",
                metadata,
            )
        return self._warning(
            RuntimeSoakCheckName.GOVERNANCE_RUNTIME,
            "Governance runtime coverage is incomplete for the soak window.",
            metadata,
        )

    def _check_replay_runtime(
        self,
        *,
        window_start: datetime,
        window_end: datetime,
    ) -> RuntimeSoakCheckResult:
        replay_runs = self._repository.list_successful_job_runs_by_names(
            job_names=("runtime_replay.trading", "runtime_replay.runtime_checks"),
            window_start=window_start,
            window_end=window_end,
        )
        debug_replay_runs = self._repository.list_successful_job_runs_by_names(
            job_names=("runtime_replay_debug.trading", "runtime_replay_debug.runtime_checks"),
            window_start=window_start,
            window_end=window_end,
        )
        metadata = {
            "runtime_replay_runs": [row.job_name for row in replay_runs],
            "debug_replay_runs": [row.job_name for row in debug_replay_runs],
        }
        if replay_runs:
            return self._passed(
                RuntimeSoakCheckName.REPLAY_RUNTIME,
                "Replay runtime service produced runtime replay job evidence.",
                metadata,
            )
        if debug_replay_runs:
            return self._warning(
                RuntimeSoakCheckName.REPLAY_RUNTIME,
                "Only debug replay evidence exists; production replay runtime did not run.",
                metadata,
            )
        return self._warning(
            RuntimeSoakCheckName.REPLAY_RUNTIME,
            "No replay runtime evidence was found in the soak window.",
            metadata,
        )

    def _check_long_running_runtime_integrity(
        self,
        *,
        window_start: datetime,
        window_end: datetime,
        ...
        cutoff = window_end - self._stale_after
        stale_jobs = self._repository.list_stale_running_runtime_jobs(cutoff=cutoff)
        stale_manifests = self._repository.list_stale_running_manifests(cutoff=cutoff)
        duplicate_running_job_names = self._repository.list_concurrent_running_job_names()
        reconciliation_snapshot_count = self._repository.count_reconciliation_snapshots(
            window_start=window_start,
            window_end=window_end,
        )
        runtime_runs = self._repository.list_runtime_job_runs(
        ...
        metadata = {
            "stale_runtime_job_ids": [row.job_run_id for row in stale_jobs],
            "duplicate_running_job_names": sorted(duplicate_running_job_names),
            "stale_manifest_run_ids": [str(row.run_id) for row in stale_manifests],
            "reconciliation_snapshot_count": reconciliation_snapshot_count,
            "runtime_run_count": len(runtime_runs),
            "missing_telemetry_window": not runtime_runs,
        }
        failed = stale_jobs or duplicate_running_job_names or stale_manifests
        if failed:
            return self._failed(
                RuntimeSoakCheckName.LONG_RUNNING_RUNTIME_INTEGRITY,
                "Long-running runtime integrity checks found stale or duplicate execution state.",
                metadata,
                severity=RuntimeSoakSeverity.CRITICAL,
            )
        gaps: list[str] = []
        if reconciliation_snapshot_count == 0:
            gaps.append("reconciliation_snapshots")
        if not runtime_runs:
            gaps.append("telemetry_window_runtime_runs")
        if gaps:
            return self._warning(
                RuntimeSoakCheckName.LONG_RUNNING_RUNTIME_INTEGRITY,
                "Long-running runtime checks found runtime evidence gaps.",
                {**metadata, "gaps": gaps},
            )
        return self._passed(
            RuntimeSoakCheckName.LONG_RUNNING_RUNTIME_INTEGRITY,
            "No orphan jobs, duplicate jobs, stale manifests, reconciliation gaps, or missing telemetry windows were found.",
            metadata,
        )
