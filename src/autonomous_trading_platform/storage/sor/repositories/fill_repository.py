...
class FillRepository(BaseRepository):
    ...
    def get_by_fill_id(self, id_value: str) -> Fill | None:
    ...
    def upsert(self, row: Fill) -> Fill:
        ...
        """
        existing = self.get_by_fill_id(row.fill_id)
    ...
    def delete_by_fill_id(self, id_value: str) -> None:
        """Delete a row by ID."""
        obj = self.get_by_fill_id(id_value)
        if obj is not None:
