...
| `atp runtime trigger-job --job-name market_ingestion_cycle` | Trigger all registry jobs that are manual-enabled. | Registry currently includes jobs the CLI cannot dispatch. | cross-domain | `run_market_ingestion_cycle`, `run_feature_pipeline_cycle`, `run_corporate_action_ingestion_cycle`, `run_experiment_pipeline_cycle`, `run_factor_exposure_monitoring_cycle`, `run_factor_neutralization_verification_cycle` | P1 |
| `atp runtime trigger-job --job-name correlation_monitoring_cycle` | Trigger correlation/covariance monitoring cycle. | Cycle exists (`run_correlation_monitoring_cycle.py`) but is not in `SCHEDULER_REGISTRY` — no trigger path exists at all. | cross-domain | Register in `SCHEDULER_REGISTRY` first, then add dispatcher | P1 |
| `atp runtime trigger-job --job-name risk_budgeting_cycle` | Trigger risk budgeting cycle. | Cycle exists (`run_risk_budgeting_cycle.py`) but is not in `SCHEDULER_REGISTRY`. | cross-domain | Register in `SCHEDULER_REGISTRY` first, then add dispatcher | P1 |
| `atp runtime trigger-job --job-name drawdown_governance_ladder_cycle` | Trigger drawdown governance ladder evaluation cycle. | Cycle exists (`run_drawdown_governance_ladder_cycle.py`) but is not in `SCHEDULER_REGISTRY`. | cross-domain | Register in `SCHEDULER_REGISTRY` first, then add dispatcher | P1 |
| `atp runtime trigger-job --job-name strategy_health_lifecycle_cycle` | Trigger strategy health lifecycle cycle. | Cycle exists (`run_strategy_health_lifecycle_cycle.py`) but is not in `SCHEDULER_REGISTRY`. | cross-domain | Register in `SCHEDULER_REGISTRY` first, then add dispatcher | P1 |
...
- `trigger-job` validates against the full scheduler registry, but the dispatcher map only covers 4 of 10 registry jobs: `trading_cycle`, `strategy_auto_promotion_cycle`, `strategy_auto_demotion_cycle`, `strategy_allocation_rebalance_cycle`. Registry jobs not dispatched: `market_ingestion_cycle`, `feature_pipeline_cycle`, `corporate_action_ingestion_cycle`, `factor_exposure_monitoring_cycle`, `factor_neutralization_verification_cycle`, `experiment_pipeline_cycle`.
- In addition, scheduler cycle files exist that are not yet in `SCHEDULER_REGISTRY` at all: `run_correlation_monitoring_cycle.py`, `run_risk_budgeting_cycle.py`, `run_strategy_health_monitor_cycle.py`, `run_drawdown_governance_ladder_cycle.py`, `run_strategy_health_lifecycle_cycle.py`, `run_trading_evaluation_cycle.py`, `run_market_backfill_cycle.py`. These cycles have no CLI trigger path, no registry entry, and are not reachable via `trigger-job`.
