...
def _build_guard() -> tuple[RuntimeTradingGuardService, RuntimeGateService]:
    settings = Settings()
...
def test_paper_mode_allowed_without_runtime_gate_arming(monkeypatch) -> None:
    _paper_env(monkeypatch)
    guard, runtime_gate_service = _build_guard()

    assert runtime_gate_service.is_armed() is False
    guard.assert_trading_mode_allowed(account_id="paper-1", run_type=RunType.PAPER)


def test_paper_mode_blocks_live_run_type(monkeypatch) -> None:
    _paper_env(monkeypatch)
...
def test_paper_mode_respects_account_allowlist(monkeypatch) -> None:
    _paper_env(monkeypatch)
    guard, _ = _build_guard()
...
def test_live_mode_blocked_by_default(monkeypatch) -> None:
    _base_env(monkeypatch)
    monkeypatch.setenv("TRADING_ENVIRONMENT", "live")
    monkeypatch.setenv("LIVE_BROKER_API_KEY", "live-key")
    monkeypatch.setenv("LIVE_BROKER_API_SECRET", "live-secret")
...
def test_live_mode_blocked_when_no_live_trading_true(monkeypatch) -> None:
...
def test_live_mode_blocked_when_enable_live_trading_false(monkeypatch) -> None:
    _live_env(monkeypatch)
    monkeypatch.setenv("ENABLE_LIVE_TRADING", "false")
    guard, _ = _build_guard()
...
def test_live_mode_blocked_when_runtime_gate_not_armed(monkeypatch) -> None:
    _live_env(monkeypatch)
    guard, _ = _build_guard()
...
def test_live_mode_allowed_only_when_all_safety_conditions_pass(monkeypatch) -> None:
    _live_env(monkeypatch)
    guard, runtime_gate_service = _build_guard()
...
def test_live_mode_respects_account_allowlist(monkeypatch) -> None:
    _live_env(monkeypatch)
    guard, runtime_gate_service = _build_guard()
    runtime_gate_service.arm(reason="supervised test", armed_by="tester")
